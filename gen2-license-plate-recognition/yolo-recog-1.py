...
def create_pipeline():
    ...
    det_nn.setNumClasses(36)
    det_nn.setCoordinateSize(4)
    det_nn.setAnchors(np.array([10, 14, 23, 27, 37, 58, 81, 82, 135, 169, 344, 319]))  # HAS PROBLEM
    det_nn.setAnchorMasks({"side26": np.array([1, 2, 3]), "side13": np.array([3, 4, 5])})
    det_nn.setIouThreshold(0.5)
    det_nn.setConfidenceThreshold(0.01)
    det_nn.setBlobPath(str(Path("models/recog.blob").resolve().absolute()))
    ...
    try:
        while should_run():
            ...
            if not args.camera:
                ...
                lic_frame.setType(dai.RawImgFrame.Type.BGRF16F16F16p)
