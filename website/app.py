...
def bk_LPR_log():
	...
	if timeline is not None:
		if timeline == 'All':
			sql_command = 'SELECT results.*, cameras.`camera_name` FROM (SELECT * FROM `result`) results LEFT JOIN cameras ON results.`camera_id` = cameras.`id` ORDER BY `created` DESC;'
		elif timeline == 'Today':
			sql_command = 'SELECT results.*, cameras.`camera_name` FROM (SELECT * FROM `result`) results LEFT JOIN cameras ON results.`camera_id` = cameras.`id` WHERE DATE(`created`) = DATE(NOW()) ORDER BY `created` DESC;'
		elif timeline == 'This Week':
			sql_command = 'SELECT results.*, cameras.`camera_name` FROM (SELECT * FROM `result`) results LEFT JOIN cameras ON results.`camera_id` = cameras.`id` WHERE WEEK(`created`) = WEEK(NOW()) and YEAR(`created`) = YEAR(NOW()) ORDER BY `created` DESC;'
		elif timeline == 'This Month':
			sql_command = 'SELECT results.*, cameras.`camera_name` FROM (SELECT * FROM `result`) results LEFT JOIN cameras ON results.`camera_id` = cameras.`id` WHERE MONTH(`created`) = MONTH(NOW()) and YEAR(`created`) = YEAR(NOW()) ORDER BY `created` DESC;'
		elif timeline == 'This Year':
			sql_command = 'SELECT results.*, cameras.`camera_name` FROM (SELECT * FROM `result`) results LEFT JOIN cameras ON results.`camera_id` = cameras.`id` WHERE YEAR(`created`) = YEAR(NOW()) ORDER BY `created` DESC;'
	else:
		sql_command = 'SELECT results.*, cameras.`camera_name` FROM (SELECT * FROM `result`) results LEFT JOIN cameras ON results.`camera_id` = cameras.`id` WHERE {} ORDER BY `created` DESC;'.format(request.args.get('where_cmd'))
